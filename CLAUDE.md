...
3. **Core Services** (`Sources/Core/`, `Sources/Status/`)
   ...
   - `ExternalChannel` — Protocol for inbound remote-control chat. `IMessageChannel` is the live one: inbound reads `~/Library/Messages/chat.db` over SQLite, edge-triggered by an FSEvents watch (needs Full Disk Access); outbound drives Messages.app via AppleScript (`IMessageSender`, needs `NSAppleEventsUsageDescription` + the `com.apple.security.automation.apple-events` entitlement in `seahelm.entitlements`, since release builds are hardened-runtime). Both directions are prefix-marked: a command must open with `sea ` and every outbound is stamped `helm ` (`IMessageConfig.commandPrefix`/`replyPrefix`). That is not cosmetic — the reader takes `is_from_me = 1` rows too, because with one Apple ID the user's own phone is the only commander there is, so seahelm's replies come back as input and the prefix is what stops the bridge answering itself. `IMessageConfig.allowedHandles` is a hard gate — empty drops everything — but it means two things: for incoming rows it matches the sender, for the user's own rows (which carry no `handle_id`) it matches the chat's other party, i.e. "which thread is a command line".
