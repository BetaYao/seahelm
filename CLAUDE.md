...
Attribution is therefore a continuous function of the agent's cwd, which is why there is **no undo**: an undo would only change seahelm's view, and the agent's next event would move the pane straight back. `seahelm pane move <pane> <worktree>` remains the manual path, and matters for panes that report no hooks at all.
