...
2. **UI Layer** (`Sources/UI/`)
   ...
   - `SidePanel/` — Per-worktree side panel with Files / Changes tabs (`WorktreeSidePanelViewController`). Changes answers "what here still has to go in a PR": uncommitted work, then committed work the base lacks. A squash merge leaves git no ancestry to go on, so committed work is counted from the last merged PR's head when GitHub has one (`MergedPRLookup`), otherwise as what `git merge-tree` says merging into the base would change (`GitDiff.branchChangedFiles`). The old First Mate orders tab (`BridgePanelViewController`) and the inline command composer (`InlineWorktreeCreateView`) are gone: pending orders and the command line both live in `Island/`.
...
3. **Core Services** (`Sources/Core/`, `Sources/Status/`)
   ...
   - `ExternalChannel` — Protocol for inbound remote-control chat. `TelegramChannel` is the live one: a Bot API bridge (`TelegramBotAPI`, blocking calls) that long-polls `getUpdates` on a thread of its own and answers with `sendMessage`. Its transport is `/usr/bin/curl` in a child process, not URLSession: in this process URLSession requests to Telegram hung after the first one or two (no response, no error, timeouts not firing) while the same requests from a standalone process or curl never failed, and per-request sessions, a watchdog and shorter polls did not cure it. The token reaches curl through a 0600 config file, never an argument. No permissions and no local app, but it needs a bot token from @BotFather (`TelegramConfig.botToken`, kept in config.json beside the pairing secret). `TelegramConfig.allowedUsers` (numeric ids or `@usernames`) is a hard gate — empty obeys nobody — and it also decides what a message *is*: from an allowed user it is an order (in a private chat any text, in a group only a `/command`, since bare prose in a shared group must not steer an agent); from anyone else it is rule input for `TelegramRuleEngine` (a monitoring channel the bot sits in, a colleague in a group). There is no echo guard because a bot's own messages never come back as updates. A forum group's **topics are separate conversations**: `TelegramChatAddress` folds the topic into the chat-id string (`-1001234567890#42`), which is what every seam downstream already treats as opaque — so two topics in one group are two `CommandSession`s with their own `/go` binding, progress line and buttons, for no change to any of them. It is split back into `chat_id` + `message_thread_id` only at the Bot API call, and a thread is recorded only for a message marked `is_topic_message` in a supergroup: `message_thread_id` is also set on a plain group's reply chain and (since Bot API 10.0) on private-chat topics, where sending it back is an error, and General — topic 1 — is addressed by leaving the thread off entirely. Fleet-wide notices and `default_chat_id` stay bare, so they land in General. An auto-opened topic covers a **worktree**, not a pane (`CommandSession.topicScope`): one per pane made the group's topic list too long to skim, so the panes in a worktree share one thread, named after `repo · branch` rather than any agent's title. `boundWorktreePath` is that topic's identity and `boundPaneKey`/`boundPaneId` are only its *current* target — `noteActivePane` repoints them to whoever reported last, so a bare order follows the work and never sticks. Once a worktree has more than one pane its notices carry a `#n` prefix (`topicLabelled`); one pane gets none. A pane ending no longer retires the thread — `close(paneId:)` just clears the pointer — and `close(worktreePath:)` from `didDeleteWorktree` is what ends it. Topics written before this decode as `.pane` and are deleted once at launch by `sweepPaneScopedTopics`. Outbound goes through `TelegramFormatter`: the bridge's light markdown to Telegram HTML, chunked to 4096, resent flat if Telegram returns 400.
...
**Terminal palette is latched for the run, not tied to app appearance:** a pane hosts long-running TUIs and a TUI reads the terminal's colors once, so the palette must never move under one. Codex asks over `OSC 10;?`/`OSC 11;?` at startup, caches the answer (failures included) for the life of the process, and does not enable DEC mode 2031 — so Ghostty's color-scheme-change report never reaches it. A flip leaves it painting from the stale reading, and since it only sets a *background* and leaves text at SGR 39, a stale Latte reading fills the user-message pill with `blend(black, #eff1f5, .04)` = `#e5e7eb` while the text stays Mocha's `#cdd6f4` — near-white on near-white. Its re-styled chrome picks up the new reading, which is how one frame carries both palettes (`#e5e7eb` pill + `#393947` composer is the fingerprint). So `terminal_theme_mode` (`app` default / `dark` / `light`, `TerminalThemeMode`) is resolved **once** per run and latched in `GhosttyBridge.terminalIsDark`; `app` reads `NSApp.effectiveAppearance` at launch, the pins ignore it. `writeTerminalThemeOverride` then writes a **single** `theme =` rather than `light:…,dark:…`, so there is no conditional state left for any `set_color_scheme` to re-resolve, and `syncColorScheme` is app-level only — it deliberately does not walk `StationRegistry` repainting live surfaces. There is no `effectiveAppearance` observer: an app theme toggle restyles Seahelm's own UI and terminal chrome (which follows the *terminal*, so it stays matched to the surface it frames); panes change palette on the next launch. Separately, a **detached** zmx session (`SessionManager.createDetachedSession`, no client attached) answers DA1 but swallows OSC 10/11 entirely, so an agent started that way caches "no colors" permanently and paints no pill background at all even after a client attaches.
