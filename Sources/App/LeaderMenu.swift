...
enum LeaderMenu {
    ...
    static let root: [LeaderNode] = [
        .submenu(key: "s", label: "split ▸", children: [
            .command(key: "s", label: "水平 split", command: .splitHorizontal),
            .command(key: "v", label: "垂直 split", command: .splitVertical),
            .command(key: "x", label: "关闭 pane", command: .closePane),
            .command(key: "=", label: "重置比例", command: .resetRatio),
        ]),
        ...
        .submenu(key: "g", label: "go ▸", children: [
            .command(key: "w", label: "worktree 切换器", command: .quickSwitcher),
            .command(key: "0", label: "dashboard", command: .dashboard),
            .command(key: "b", label: "切 sidebar", command: .toggleSidebar),
        ]),
        .submenu(key: "w", label: "window/pane ▸", children: [
            .command(key: "H", label: "缩放 ←", command: .resize(.left)),
            .command(key: "J", label: "缩放 ↓", command: .resize(.down)),
            .command(key: "K", label: "缩放 ↑", command: .resize(.up)),
            .command(key: "L", label: "缩放 →", command: .resize(.right)),
            .command(key: "m", label: "最大化 pane", command: .maximizePane),
        ]),
