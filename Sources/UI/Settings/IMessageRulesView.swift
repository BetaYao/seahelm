...
final class IMessageRulesView: NSView {
    ...
    private static let anyWorktree = "Any worktree in this project"
    private static let anyPane = "Any pane in this worktree"
    ...
    private func build() {
        ...
        for (popup, id) in [(projectPopup, "Project"), (worktreePopup, "Worktree"),
                            (panePopup, "Pane")] {
            popup.target = self
            popup.action = #selector(targetChanged)
        ...
        formRows = [
            ...
            SettingsRow.make("Project", control: projectPopup),
            SettingsRow.make("Worktree", control: worktreePopup),
            SettingsRow.make("Pane",
                             subtitle: "Narrow as far as you want: stop at the project or worktree and the rule takes the first pane there, which survives panes being closed and reopened.",
                             control: panePopup),
        ]
    ...
    private func rebuildTargetPopups(for target: IMessageRuleTarget) {
        ...
        fill(projectPopup, items: projects, select: selectedProject,
             emptyTitle: "No running panes")

        let project = projectPopup.titleOfSelectedItem ?? ""
        var worktrees = Array(Set(panes.filter { $0.project == project }.map(\.worktreePath)))
            .sorted()
        ...
        if !selectedWorktree.isEmpty, !worktrees.contains(selectedWorktree) {
            worktrees.insert(selectedWorktree, at: 0)
        }
        fill(worktreePopup, items: [Self.anyWorktree] + worktrees.map(displayPath),
             select: selectedWorktree.isEmpty ? Self.anyWorktree : displayPath(selectedWorktree),
             emptyTitle: Self.anyWorktree)

        let worktree = resolvedWorktreePath()
        var paneKeys = panes.filter { $0.worktreePath == worktree }
            .map(\.paneSessionKey)
            .filter { !$0.isEmpty }
        let selectedPane = target.kind == .pane ? stored : ""
        if !selectedPane.isEmpty, !paneKeys.contains(selectedPane) {
            paneKeys.insert(selectedPane, at: 0)
        }
        fill(panePopup, items: [Self.anyPane] + paneKeys,
             select: selectedPane.isEmpty ? Self.anyPane : selectedPane,
             emptyTitle: Self.anyPane)

        worktreePopup.isEnabled = !projects.isEmpty
        panePopup.isEnabled = worktree != nil
    }

    private func fill(_ popup: NSPopUpButton, items: [String], select: String, emptyTitle: String) {
        popup.removeAllItems()
        popup.addItems(withTitles: items.isEmpty ? [emptyTitle] : items)
        if !select.isEmpty, popup.itemTitles.contains(select) {
            popup.selectItem(withTitle: select)
        }
    }

    /// Worktree paths are long and their tail is the informative part.
    private func displayPath(_ path: String) -> String {
        (path as NSString).abbreviatingWithTildeInPath
    }

    private func resolvedWorktreePath() -> String? {
        guard let title = worktreePopup.titleOfSelectedItem, title != Self.anyWorktree else {
            return nil
        }
        // The popup shows a tilde-abbreviated path; the rule stores the real one.
        return panes.map(\.worktreePath).first { displayPath($0) == title } ?? title
    }
    ...
    @objc private func targetChanged() {
        ...
        let paneKey = panePopup.titleOfSelectedItem
        let worktree = resolvedWorktreePath()
        let project = projectPopup.titleOfSelectedItem ?? ""
        ...
        if let paneKey, paneKey != Self.anyPane {
            target = IMessageRuleTarget(kind: .pane, value: paneKey)
        } else if let worktree {
            target = IMessageRuleTarget(kind: .worktree, value: worktree)
        } else {
            target = IMessageRuleTarget(kind: .project, value: project)
        }
    ...
    private func currentTarget() -> IMessageRuleTarget {
        if let paneKey = panePopup.titleOfSelectedItem, paneKey != Self.anyPane {
        ...
        return IMessageRuleTarget(kind: .project, value: projectPopup.titleOfSelectedItem ?? "")
    }
...
extension IMessageRulesView: NSTableViewDataSource, NSTableViewDelegate {
    ...
    func tableView(_ tableView: NSTableView,
        ...
        let targetLabel = (rule.target.value as NSString).lastPathComponent
