...
├── pane/{pane_session_key}/              # 段 = 稳定槽位键 pane_session_key(跨重启不变),非易变 pane_id UUID
...
- **retained 清理**:pane 关闭发**零长度 retained** 墓碑,避免幽灵 pane 常驻。触发点 `ShipLog.unregister` → EventHub `pane.closed` → `MqttChannel` 清 `pane/{pane_session_key}/status`。
- **pane 寻址键**:pane topic 段 = `pane_session_key`(稳定槽位,重启覆盖同一 topic → 无孤儿);`command` / `history/request` 也用 `pane_session_key` 定位 pane,`MqttChannel` 内部再解析成易变的 `pane_id`(UUID)交给 `ControlRouter`。旧客户端仍可传 `pane_id`(向后兼容)。`pane_id` 只在 payload 里供调试。

---

## 3. Payload 契约(复用现有序列化,不另立 schema)

- **`pane/{id}/status`** = `PaneSnapshot.dict`(`Sources/Core/ControlProtocol.swift:24`):`pane_id`/`pane_session_key`/`worktree_path`/`branch`/`project`/`agent_type`/`status`/`last_message`,发布侧补 `seq`。
- **`pane/{id}/message` / `event`** = `ShipLog.event(from:)`(`Sources/Core/ShipLog.swift:382`):`type`/`seq`/`pane_id`/`pane_session_key`/`status`/`old_status`/`agent_type`/`worktree_path`/`last_message`。
...
## 7.5 认证 + E2EE 定稿(2026-07,统一四端:Web / iOS / Watch / ESP32)

> 本节是 §6/§7 之上的落地定稿。目标:**一套简单方案覆盖四端**,并在 broker 认证之上再加一层
> **端到端加密**,使 broker 运营方(EMQX Cloud)也看不到终端内容。

### 7.5.1 安全分层(E2EE 是最内层)

| 层 | 防的是谁 | 手段 |
|---|---|---|
| TLS(wss/mqtts) | 网络监听者 | 传输加密(§6.1) |
| Broker 认证 + ACL | 未配对的人 | 每 Mac 一密钥 + `{mac_id}` 命名空间锁定(§6.2/§6.3) |
| **E2EE(本节新增)** | **broker 运营方本身** | payload 用共享密钥 AES-256-GCM 加密 |

### 7.5.2 配对载荷(单一规范,多种传输)

配对载荷永远是同一份三元组,渲染成不同形态送进不同设备:

```
seahelm://pair?b=<broker_url>&m=<mac_id>&k=<base64url root_secret>
```

- `root_secret`:每台 Mac 随机生成(存 config.json / Keychain),**用 HKDF-SHA256 派生两把子密钥**:
  - `k_auth` → MQTT broker 密码(第 2 层认证)
  - `k_enc` → 32 字节 AES-256-GCM 密钥(第 3 层 E2EE,**永不上 broker**)
- `username` = `mac_id`(或固定值);ACL 锁 `seahelm/<mac_id>/#`。

### 7.5.3 E2EE 信封

对**所有 payload body 统一加密**(status JSON 也加;topic 保持明文用于路由):

```
envelope = version(1B) | nonce(12B, 随机) | ciphertext+tag   (AES-256-GCM)
AAD = topic 字符串                        // 把密文绑定到主题,防跨主题重放
```

- nonce 每条随机 96-bit;随机 nonce 下安全到 ~2³² 条/密钥,重新配对即换 `k_enc`。
- retained 消息存的是密文 → retained 模型不受影响。
- 四端原生 crypto,无第三方库:Mac/iOS/Watch = CryptoKit `AES.GCM`;Web = WebCrypto SubtleCrypto;ESP32 = mbedTLS `mbedtls_gcm`(硬件加速)。

**固有取舍(接受)**:①topic 名明文 → broker 可见"结构性元数据"(有哪些 pane/状态在变),但看不到内容;②共享对称密钥 → 无单设备撤销、无前向保密,撤销任一设备需重新配对全部端换 `k_enc`。要单设备撤销/前向保密需 per-device 密钥 + 按订阅方分别加密,复杂度陡增,**一期不做**。

### 7.5.4 macOS 配对窗口(状态机)

窗口内三件东西,按"通道强弱 + 是否常驻"区分:

| 元素 | 内容 | 常驻 | 用途 |
|---|---|---|---|
| **二维码** | 完整载荷(含真 `root_secret`) | 打开即在 | iOS 扫;手机代扫给 ESP32 |
| **长链接** | 同一份完整载荷 | 打开即在 | Web 粘贴 / 一次性 localhost 链接 |
| **[生成短码]** | **不含 secret**,只开一个握手会话 | **点了才有,1 分钟过期,单次使用** | Watch、无相机端 |

- 短码是最弱通道(6–8 位可暴力枚举),故**按需才存在**:点按钮 → 起限次数 + 短过期握手监听 → 到期/成功即拆掉;不点则无监听端点,攻击面为零。
- 反直觉点:二维码/长链接含真 secret 且只要窗口开着就明晃晃挂屏、不过期 → 泄露后果反而更重。故:**配对成功后或超 N 分钟自动关窗/打码**;二维码/长链接窗口开着期间可复用(多端扫同一个),短码单次使用。
- 短码握手换 secret 必须走 **PAKE(SPAKE2)** 或**本地端点 + 限次**,禁止在公网 broker 上枚举换 secret。

### 7.5.5 四端配对通道

| 设备 | 依赖 iOS | 通道 |
|---|---|---|
| **Web** | 否 | 复制长链接粘贴 / 一次性 localhost 链接;存 localStorage |
| **ESP32** | 否 | SoftAP 或 BLE provisioning,与 WiFi 配网合并(ESP-IDF `wifi_provisioning` 自带 X25519+AES 信道);存 NVS |
| **iOS** | —(可后补) | 相机扫 QR → Keychain。**是可选的扫码快捷方式,非必需环节** |
| **Watch** | 否(改用短码) | watchOS 无相机:数字表冠输**短码** → 短码握手换 `root_secret`。若已有 iOS,可改为 WatchConnectivity 从配对 iPhone 继承 |

**关键:配对权威在 Mac,不依赖 iOS。** iOS 未开发不阻塞其它端——Web/ESP32 独立,Watch 走短码握手独立配对。落地顺序建议:**Web(基本现成)→ ESP32 → Watch(短码握手)→ iOS(后补扫码)**。
...
|---|---|---|---|---|
| 3 | **直接给 pane 下命令/文本** | 发 `command`(`pane.send_text`,Control 档) | `ControlRouter.pane.send_text` **现成**(`ControlProtocol.swift:183`) | **≈0**。设备用 retained `pane/+/status` 拿 `pane_session_key` 作目标 |
...
```jsonc
{ "type":"pane.status_changed", "seq":1042, "pane_id":"p3", "pane_session_key":"…",
...
### 15.4 字段参考(逐字对齐源码)
- `PaneSnapshot.dict`:`pane_id`·`pane_session_key`·`worktree_path`·`branch`·`project`·`agent_type`·`status`·`last_message`(+发布侧 `seq`)。
- `ShipLog.event(from:)`:`type`(`pane.status_changed`|`pane.updated`)·`seq`·`pane_id`·`pane_session_key`·`status`·`old_status`·`agent_type`·`worktree_path`·`last_message`。
