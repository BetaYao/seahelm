...
final class TelegramRuleTargetTests: XCTestCase {
    ...
    func testPickingAWorktreeSticks() {
        ...
        let worktreePopup = popup(view, "settings.telegram.ruleTargetWorktree")
        ...
        XCTAssertEqual(view.rules[0].target, TelegramRuleTarget(kind: .worktree, value: "/work/b"))
    }
    ...
    func testPickingAWorktreeClearsAStalePaneTarget() {
        ...
        XCTAssertEqual(view.rules[0].target, TelegramRuleTarget(kind: .worktree, value: "/work/a"))
    }
    ...
    func testPickingAnyWorktreeFallsBackToTheProject() {
        ...
        XCTAssertEqual(view.rules[0].target,
                       TelegramRuleTarget(kind: .project, value: "saas-mono"))
    }
    ...
    func testPickingAPaneStoresItsSessionKeyNotItsTitle() {
        ...
        let panePopup = popup(view, "settings.telegram.ruleTargetPane")
        panePopup.selectItem(at: 2)   // the second same-titled pane
        panePopup.sendAction(panePopup.action, to: panePopup.target)

        XCTAssertEqual(view.rules[0].target, TelegramRuleTarget(kind: .pane, value: "k2"))
    }
}
